...
case "${TOOL_NAME:-}" in
  Bash)
    ...
    if printf '%s' "$CMD" | grep -qE "pbcopy|osascript.*(paste|keystroke)"; then
      ...
      echo "   → Pasting AI content into a Doc/Slack/Notion? Prepend '<!-- ai-suggestion:unverified -->' and highlight #FEB4DC."
    fi ;;
  mcp__google-sheets__update_cells|mcp__google-sheets__add_rows|mcp__google-sheets__batch_update_cells|mcp__google-sheets__batch_update|mcp__google-sheets__add_columns)
    ...
    echo "   → Populate the :Provenance column with 'ai-suggestion:unverified {YYYY-MM-DD} {short-id}' (or ai-processed:unverified); highlight touched cells #FEB4DC." ;;
esac
exit 0
