...
This file is `@import`ed into your CLAUDE.md. It is the behavioral subset of the standard; the full reference is in `../spec/SPEC.md` and `../spec/adversarial-gate.md`. Personal overrides live in `user.local.md` (gitignored) and **extend** this file — they do not replace it.
...
- Markdown: `.ai.md` + frontmatter `<!-- ai-suggestion:unverified | session:<id> | date:<YYYY-MM-DD> -->`. Other media (Sheets, Docs, Slack, code): see SPEC §5. Pink `#FEB4DC` highlight = AI/unverified; removed when verified.
